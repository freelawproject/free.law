...
export default function Button({children, href, extraClasses, size}) {
  let classes ="text-center whitespace-nowrap border border-transparent rounded-md shadow-sm text-base font-medium text-white text-sm no-underline"
...
export function WideButton({children, href, extraClasses}){
  ...
  if (extraClasses) {classes += " " + extraClasses}
  return <Button href={href} extraClasses={classes}>{children}</Button>
}
...
export function RedButton({children, href, extraClasses, size}){
  ...
  if (extraClasses) { classes += " " + extraClasses}
  return <Button href={href} extraClasses={classes} size={size}>{children}</Button>
}

const whiteClasses = "text-gray-800 border-gray-700 hover:border-gray-900 hover:text-gray-900 bg-white"
...
export function WideWhiteButton({children, href, extraClasses}){
  let classes = whiteClasses
  if (extraClasses) {classes += " " + extraClasses}
  return <WideButton href={href} extraClasses={classes}>{children}</WideButton>
}
