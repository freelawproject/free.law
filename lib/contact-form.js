...
export function validateSubmission(data) {
  ...
  if (issue === 'partnerships') {
    ...
    if (backgrounds.length === 0) {
      errors.partner_background_other = 'Please specify at least one background.';
    } else if (
...
// The 3-digit subject honeypot: a subject starting with three digits is treated
// as spam (the field is mislabeled "phone_number" to bait autofill bots).
export function looksLikeSpam(data) {
  return /^\d{3}/.test(String(data.phone_number || ''));
}
