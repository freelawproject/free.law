...
No. Due to technical limitations, RECAP does not currently upload appellate case dockets or documents purchased from US Courts of Appeal PACER sites. *(While we hope to restore this feature in the future, no timeline for doing so is available.)*
...
We currently support Firefox and Chrome, with an experimental version for Opera. We would love to also add support for Safari and *(chromium-based)* Edge, but we need a maintainer that is willing to help with these platforms when new versions are released. If that might be you, [send us a message][c]!
