...
#### Will RECAP work on *any* PACER site?
No. We have not been able to summon the necessary resources to build and test a version of RECAP for the federal appellate courts. We hope to restore this feature in the future, but do not have a timeline for doing so.
...
Second, if you wish to disable RECAP completely, you can do so via Chrome, Firefox or Safari's extension settings.

#### What browsers are supported? Why?
We currently support Firefox, Chrome and Safari (iOS and macOS). Microsoft Edge is in beta. We have experimented with Opera, but found their extension store to be too buggy and restrictive to use productively.
...
Finally, as noted above, RECAP has not supported the US Courts of Appeal [since late 2017][new-recap]. When using one of their PACER sites with RECAP, dockets and documents are currently unable to be contributed to *(or downloaded from)* our archive.
Note: Some case data from 2017 and earlier, in addition to more recent information that we have obtained from other sources, *may* be available by manually searching that archive.
