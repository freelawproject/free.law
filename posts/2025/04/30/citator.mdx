...
To a human, this seems straightforward, but in the text of a decision, a citation can take many forms. For example:
 ...
 - **A complete citation:** XXX
 - **Just by a "short name":** XXX
 - **By *id* or *supra*: XXX
...
When a panel of judges hears a case, like at the Supreme Court, you wind up with several opinions that go together, such as the lead decision, a concurence, and a dissent. As legal researchers know from experience, identifying the opinion type is important for analyzing it's precedential status.
...
To account for this, we identify the different opinion types in Courtlistener and extract the passages surrounding each opinion type, prepending the opinion types to each extracted passage.
...
<!-- say what we did for each model here, right? -->


## Optimizing Performance Through Advanced Prompting

Finally, to get the best results from our models, we developed special prompting techniques that help understand legal language better. Here's what we did:
