...
For more than a decade, we and others have been writing about the many reasons that the PACER and CM/ECF system must be overhauled. We have laid out the numerous reasons why Congress needs to pass the [Open Courts Act ("OCA")][oca] so that we can finally fix the [PACER Problem][problem].
...
In 2017, we became concerned about the security of the system when we discovered [a frighteningly basic vulnerability][vuln] in it. The vulnerability allowed the malicious owner of a website you visited to purchase content from PACER using your logged-in account. If the owner of such a website did so, they could spend your money or download your confidential sealed documents. Imagine an accused criminal luring a prosecutor onto a malicious website in order to download confidential documents from their case.

This type of vulnerability [is considered "critical"][owasp] because it is trivial for security auditors or adversaries to discover and exploit. In general, if you have this type of vulnerability, your whole approach to cybersecurity is suspect.

At the time we reported this vulnerability, it took the Judiciary six months to fix it. This is far too long. In the aftermath, we [urged the judicial branch][vuln] to centralize PACER, to make it free, and to rebuild it with modern best practices — all key components of the [Open Courts Act][oca].
...
<AlertBox>
  ...
  <p>PACER and CM/ECF are the online system used by the public and the Judiciary to manage cases and file and download documents in the federal courts. Every day, this system is used by court staff, journalists, legal startups, researchers, and the public. It is used to upload document into cases, schedule courtrooms, complete complex research topics, write groundbreaking articles, and much more.
  </p>
</AlertBox>
...
<blockquote>
  ...
  CM/ECF software is built, deployed, and maintained. <strong>Security and compliance</strong> are
...
Last year, before the SolarWinds hack and before the "Path Analysis" was released, the House of Representatives passed [the Open Courts Act][oca] by unanimous consent. We ran out of time to get it through the Senate.
