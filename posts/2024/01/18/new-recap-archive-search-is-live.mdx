...
<p class="lead">In 2016, <a href="/2016/11/22/launching-a-new-recap-archive">we launched a major new search engine for the RECAP Archive</a>. Since then, it has grown in usage and size, measuring in at over four-hundred-million items and receiving about thirty-thousand queries per day.</p>

Today after nearly a year of work, and [several months in beta][beta], we're launching a brand-new search engine for the [RECAP Archive][archive], switching the underlying system to be more accurate, scalable, and functional. 
...
1. **All Cases Are Now Searchable.** 
    ...
    The old search system primarily searched *filings,* instead of *cases.* But unfortunately, due to the [PACER paywall][paywall], many cases in our system don't have any filings, meaning that such cases could not show up in search results. 

    That was a big problem that is now fixed. Now, if you search the name of a case that lacks filings, you'll see something like this:
    ...
    }}>Search results with no filings in the new system.</CaptionedImage>
...
2. **You can search for *exactly* the words you want.**
    ...
    In general, people want searches to be broadly interpreted. For example, if you search for `immigration`, you probably want to also get results for `immigrate`, `immigrates`, `immigrated`, etc. But sometimes that can be annoying, and people have often asked if it could be disabled. 

    In the new search engine, surround any word in double quotes, and we will use just that term and not any variation of it. Think: "[Deposit" vs. "Deposition][dminusd]" or "McDonald" the last name vs. "McDonalds" the restaurant (plural).

3. **Acronyms!**

   It's frustrating when searches for common acronyms and abbreviations don't bring back their longer forms. To fix this, we have searched our data and the Blue Book to find the top 800 legal abbreviations and acronyms. Try search for things like ["IRS"][irs] to see this in action. 

4. **Emojis**
 
    Call it the [Eric Goldman][eg] feature: You can now search emojis and unicore characters. 
...
In short, we've attempted to fix every bug and feature request people have sent us. 

This makes it the best search engine we've ever released. 
...
In the coming months we will be building on this foundation:
 ...
 - We will be build a new high-speed alert system for RECAP to send notifications when search queries have new results.
